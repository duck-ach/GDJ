...
//		String authCode = securityUtil.generateRandomString(6); // 두번째 방법 (스프링이 해주는 것)
		 ...
		 * 
		 * 1. 구글 로그인 2. Google 계정 - 보안 1) 2단계 인증 - 사용 2) 앱 비밀번호 (1) 앱 선택 : 기타 (2) 기기 선택
		 * : Windows 컴퓨터 (3) 생성 버튼 : 16자리 앱 비밀번호를 생성해 줌(이 비밀번호를 이메일 보낼 때 사용)
		 */
		...
		try {
			...
			// 본문
			message.setContent("인증번호는<strong>" + authCode + "</strong>입니다.", "text/html; charset=UTF-8"); // MIME-TYPE을
																											// 적어준다.
	...
	public void keepLogin(HttpServletRequest request, HttpServletResponse response) {
		 ...
		 * 로그인 유지를 체크한 경우
		 * 
		 * 1. session_id를 쿠키에 저장해둔다. (쿠키명 : keepLogin) 2. session_id를 DB에 저장해 둔다.
		 * (SESSION_ID 칼럼에 session_id를 저장하고, SESSION_LIMIT_DATE 칼럼에 로그인유지가 언제까지 유효한지(세션
		 * 만료일)를 (나는 15일 후 날짜)저장
		 * 
		 * 로그인 유지를 체크하지 않은 경우 1. 쿠키 또는 DB중 하나만 삭제하면 된다. (둘다 매칭해서 같은 경우 저장된것을 꺼내오는
		 * 시스템이라서) 편의상 쿠키명 keepLogin을 제거하는 것으로 처리한다.
		 * 
		 */
		String id = request.getParameter("id");
		...
		if (keepLogin != null) { // 로그인 유지를 체크한 경우 : keep
			...
			UserDTO user = UserDTO.builder().id(id).sessionId(sessionId) // timeStamp 1/1000 이므로, timeStamp에 15일을 더한 값
	...
	public void restoreUser(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (insertCount > 0 && deleteCount > 0) {
				...
				out.println("<script>");
				out.println("alert('휴면계정이 복구되지 못했습니다. 다시 시도해 주세요.')");
				out.println("history.back();");
				out.println("</script>");
	...
	public String getNaverLoginApiURL(HttpServletRequest request) {
		String apiURL = null;
		try {
			String clientId = "_8UV92ISq0zjxbDv1BUf";// 애플리케이션 클라이언트 아이디값";
			String redirectURI = URLEncoder
					.encode("http://localhost:9090" + request.getContextPath() + "/user/naver/login", "UTF-8");
	...
	public UserDTO getNaverLoginTokenNProfile(HttpServletRequest request) {
		 	...
		 	String clientId = "_8UV92ISq0zjxbDv1BUf";//애플리케이션 클라이언트 아이디값";
		    String clientSecret = "NS8cKmmUZ3";//애플리케이션 클라이언트 시크릿값";
