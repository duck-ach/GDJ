...
<script>
	...
	function fn_idCheck(){
		$('#id').keyup(function(){
			...
			// 중복체크(요청뿐만 아니라 응답까지 받아와야해서 ajax 이용)
			$.ajax({
				/* request */
				type: 'get',
				url : '${contextPath}/user/checkReduceId',
				data: 'id=' + $(this).val(), // id에 입력된 값 파라미터로 넘기기
				/* response */
				dataType: 'json',
				success: function(resData){ // resData = {"isUser" : true, "isRetireUser" : false}
					if(resData.isUser || resData.isRetireUser){ // 현재 회원이거나 탈퇴한 회원이라면
	...
	/* 
		Promise : 자바스크립트 비동기 처리에 사용되는 객체. 주로 서버에서 받아온 데이터를 화면에 표시할 때 사용함
	...
	*/
	function fn_emailCheck(){ 
		$('#btn_getCode').click(function(){
			new Promise(function(resolve, reject) { // resolve : 성공하면 수행할 function, reject : 실패하면 수행할 function
				$.ajax({
					/* request */
					type: 'get',
					...
					success: function(resData) {
						if(resData.isUser) {
							reject(); 	// 기존의 사용자 정보면 실패.Promise 객체의 catch 메소드에 바인딩되는 함수
						} else {
							resolve(); 	// 기존의 사용자 정보가 아닐 때 성공. Promise 객체의 then 메소드에 바인딩되는 함수
						}
					}
				}); // ajax
			}).then(function(){
				// 인증번호 보내는 ajax (자바코드이기 때문에 ajax 처리 해야함)
				$.ajax({
					/* request */
					type: 'get',
					...
					/* response */
					dataType: 'json',
					success : function(resData) { // Map에 인증코드(Authcode)가 들어있음 resData.authCode
				...
				$('#authCode').prop('readonly', true); // 인증코드를 입력할 수 없게 막았다.
			}); // new Promise
		});
	}
	...
	<div>
		...
		<form id="frm_join" action="${contextPath}/user/join" method="post">
			...
			<div>
				...
				<input type="button" value="인증번호받기" id="btn_getCode">
