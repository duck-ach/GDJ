...
<script>
   ...
   // 전역변수 - 전역변수의 위치는 함수 밖에 있어야 함
   var pwPass = false;
   var rePwPass = false;
   
   function fn_showHide(){
      
      $('#modify_pw_area').hide();   // 평소에 숨어있다
      
      $('#btn_edit_pw').click(function() {  // 변경 버튼 클릭하면 보여준다
         $('#modify_pw_area').show();
      });
      
      $('#btn_edit_pw_cancel').click(function(){ // 취소버튼 누르면 초기화&닫아준다
         $('#modify_pw_area').hide();
      });
   ...
   function fn_pwSubmit() {
      ...
      $('#frm_edit_pw').submit(function(event){   // 조건이 틀렸으면 비밀번호 변경 못하게 막음
