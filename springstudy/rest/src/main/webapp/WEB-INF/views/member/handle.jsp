	...
	function fn_modify(){
		$('#btn_modify').click(function(){
			...
			let member = JSON.stringify({
				...
				name: $('#name').val(),
				gender: $(':radio[name=gender]:checked').val(), // 체크된 애의 N or F
				address : $('#address').val()
			});
		...
		$('#btn_remove').click(function(){
			if(confirm('선택한 회원을 모두 삭제할까요?')) {
				...
				for(let i = 0; i < $('.check_one').length; i++) {
					if($($('.check_one')[i]).is(':checked') ) { // 본래 jquery이지만, [i]배열로 빼면 한 요소가 javascript이기 때문에 한번 더 감싸준다.
