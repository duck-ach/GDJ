...
@Component // bean으로 만들어짐 / RequestLoggingAspect 클래스를 Bean으로 만들어 두시오.
...
public class RequestLoggingAspect {
	...
	// HomeController에서는 이미 LOG를 찍을 수 있게 세팅이 되어있었음.
	
	@Pointcut("within(com.gdu.app08.controller..*)") // controller 패키지 아래 모든 패키지(..)의 모든 클래스
	// 컨트롤러의 모든 메소드를 포인트컷으로 지정하겠다는 뜻
	// 컨트롤러의 모든 메소드에서 어드바이스(콘솔에 로그 찍기)가 동작한다. 
	// controller에서 모든 메소드 index()와 같은 것. = JoinPoint
	// 실제로 적용시키고자 하는 메소드 = PointCut
	// JoinPoint >= PointCut 
	public void setPointCut() {} // 오직 포인트 컷 대상을 결정하기 위한 메소드(이름 : 아무거나, 본문 : 없음)
