...
public class DBConfig {
   ...
   // Transaction interCeptor를 Bean으로 등록한다.
   @Bean
   public TransactionInterceptor transactionInterceptor() { // advice : rt
	   
	   // 모든 Exception이 발생하면 Rollback을 수행하시오.
	   RuleBasedTransactionAttribute attribute = new RuleBasedTransactionAttribute();
	   attribute.setName("*");
	   attribute.setRollbackRules(Collections.singletonList(new RollbackRuleAttribute(Exception.class))); // 모든 예외가 발생하면 
	   MatchAlwaysTransactionAttributeSource source = new MatchAlwaysTransactionAttributeSource(); // 클래스이기때문에 그대로 생성
