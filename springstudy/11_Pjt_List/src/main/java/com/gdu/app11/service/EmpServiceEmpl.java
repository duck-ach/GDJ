...
public class EmpServiceEmpl implements EmpService {
	...
	private EmpMapper empMapper;
   	...
   	public void findAllEmployees(HttpServletRequest request, Model model) {
        ...
        // 뷰로 보낼 데이터 (== request.setAttribute)
        model.addAttribute("employees", employees);
        model.addAttribute("paging", pageUtil.getPaging(request.getContextPath() + "/emp/list"));
        model.addAttribute("beginNo", totalRecord - (page - 1) * pageUtil.getRecordPerPage()); // 1페이지는 107이 시작, 2페이지는 97이 시작
        ...
        /*
        	주소예시
        	/app11/emp/serarch?column=EMPLOEE_ID&query=150
        */
