...
public class BoardDao {
	...
	public List<Board> selectAllBoards() {
		...
		List<Board> boards = ss.selectList("mybatis.mapper.board.selectAllBoards"); // mybatis.mapper.board 매퍼의 selectAllBoards 아이디를 가진 쿼리문
		ss.close();
		return boards;
	}
	...
	public int insertBoard(Board board) {
		...
		int result = ss.insert("mybatis.mapper.board.insertBoard", board); // board를 파라미터로
		if(result > 0) {
			ss.commit();
		}
		return result;
