...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		String realPath = getServletContext().getRealPath("upload"); // src/main/webapp(web root) 아래 storage 디렉터리
		...
		MultipartRequest multipartRequest = new MultipartRequest(
					...
					/* 최대 크기*/			 1024 * 1024 * 10,
					/* 인코딩 */			 "UTF-8",
					/* 파일명 중복 처리 */	 new DefaultFileRenamePolicy() // 파일명이 같다면 file 이름을 새로 지어줌
											);
		
		// 5. 업로드 결과 확인
		//    일반 리퀘스트인 HttpServletRequest는 사용할 수 없음 (파라미터는 name값)
