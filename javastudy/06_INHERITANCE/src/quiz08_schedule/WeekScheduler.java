		...
		for(int i = 0; i < week.length; i++) {
			if(dayName.equals(dayNames[i])) {
				if(week[i] == null) { // 등록된 스케쥴이 없으면
					...
					String schedule = sc.nextLine(); // 스케쥴에 공백입력이 가능하다.
					Day day = new Day();
					day.setSchedule(schedule);
					week[i] = day;
					System.out.println("■■" + dayName + "요일에 " + schedule + " 스케줄이 등록되었습니다. ■■");
				} else {
					System.out.println("■■" + dayName + "요일은 이미 스케쥴이 있습니다. ■■");
				}
				return; // 해당하는 요일이 있는 경우
				// 밑에있는 코드를 실행하지않고 메소드 종료
			}
		}
		// 해당하는 요일이 없을 경우
			...
			if(dayName.equals(dayNames[i])) {
				if(week[i] == null) {
					...
					if(yesNo.equalsIgnoreCase("y")) {
						...
						System.out.println("■■" + dayName + "요일의 새 스케줄 등록이 취소되었습니다. ■■");
					}
				} else {
