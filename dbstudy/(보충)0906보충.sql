...
RETURN NUMBER -- 반환하는 학번의 타입이 NUMBER(5)이므로, 사이즈는 전달하지 않음.(VARCHAR2타입도 포함)
IS
...
-- 트리거
-- SAMPLE 테이블의 행을 갱신, 삭제하면 'SampleTrg 동작' 서버메시지 출력하기
CREATE OR REPLACE TRIGGER SampleTrg
    AFTER -- 이후인지 이전인지 여부
    UPDATE OR DELETE -- 갱신하고 삭제
    ON SAMPLE -- 테이블명
    FOR EACH ROW
BEGIN
...
SELECT S.NAME
  ...
  FROM STUDENT S INNER JOIN DEPARTMENT D
    ON S.DEPT_NO = D.DEPTNO INNER JOIN PROFESSOR P
    ON P.DEPT_NO = D.DEPTNO;
