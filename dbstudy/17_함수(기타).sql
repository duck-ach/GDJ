-- 1. 순위
--    1) RANK() OVER(ORDER BY 순위구할 칼럼 ASC)  : 오름차순 순위, 낮은 값이 1등, ASC는 생략가능
--    2) RANK() OVER(ORDER BY 순위구할 칼럼 DESC) : 내림차순 순위, 높은 값이 1등
...
-- 2) EMPLOYEES 테이블의 사원 정보를 입사 순으로 조회하기
SELECT
        RANK() OVER(ORDER BY HIRE_DATE) AS 입사날짜
      , EMPLOYEE_ID
      , FIRST_NAME
      , LAST_NAME
      , SALARY
...
--     그룹화 작업을 수행하므로 집계함수(그룹함수)와 함께 사용이 가능하다.
SELECT 
    DISTINCT DEPARTMENT_ID 
  , SUM(SALARY) OVER(PARTITION BY DEPARTMENT_ID) AS 부서별연봉합계
  , FLOOR(AVG(SALARY) OVER(PARTITION BY DEPARTMENT_ID)) AS 부서별연봉평균 -- FLOOR는 OVER문까지 감싸서 처리
...
--    , 값3, 결과3
--    , ...)
--   표현식의 결과가 값1이면 결과1반환, 값2이면 결과2 반환, ...
--   표현식의 결과와 값의 비교는 동등 비교(=)만 가능함

-- JOIN없이 EMPLOYEES 테이블만 이용하여 EMPLOYEE_ID, DEPARTMENT_NAME 조회하기
-- DB성능을 위해 사람이 노가다함
SELECT
    EMPLOYEE_ID
...
-- 603          SOUTH
-- 650          NORTH
-- EMPLOYEES테이블에 PHONE_NUMBER가 이렇게 6개로 시작한다.
...
SELECT EMPLOYEE_ID AS 사원번호, TO_CHAR(HIRE_DATE, 'YYYY-MM-DD') AS 고용날짜, TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS 근무개월수,
    CASE WHEN MONTHS_BETWEEN(SYSDATE, HIRE_DATE) >= 240 THEN '정산대상'
    ELSE ''
    END AS 퇴직금정산대상 
    FROM EMPLOYEES;