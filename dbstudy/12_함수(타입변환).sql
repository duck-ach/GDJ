/*
    ...
    3. 오라클에서는 FROM절이 필수이기 때문에,
       테이블이 필요 없는 조회문에서 DUAL 테이블을 사용한다.
*/

DESCRIBE DUAL; -- DESC DUAL
...
가장 성능이 낮은 것은?
    ...
    성능이가장낮은것은 1.2번이다.
    이유 :
    1) 왼쪽에는 되도록이면 함수나 계산식을 넣지 않는것이 좋다.
    2) 왼쪽과 오른쪽의 데이터를 맞추는 것이 성능이 좋다.
*/

-- 2. 문자로 변환(TO_CHAR)
SELECT
      TO_CHAR(123)
    , TO_CHAR(123, '999999')   --    123 혹시 숫자가 부족하면 ' ' 빈칸
    , TO_CHAR(123, '000000')    -- 000123 혹시 숫자가 부족하면  0 
    , TO_CHAR(1234, '9,999')   -- 1,234  
    , TO_CHAR(12345, '9,999')  -- ###### 4자리를 출력하라고 했으나 값은 5자리여서 나타낼 수 없다.
    , TO_CHAR(12345, '99,999') -- '12,345'
    , TO_CHAR(1.4, '9')        -- 형식은 정수 1자리 표기(소수 이하 반올림)
    , TO_CHAR(1.5, '9')        -- 형식은 정수 1자리 표기(소수 이하 반올림)
    , TO_CHAR(0.123, '0.00')    -- '0.12', 소수 이하 2자리 표기
    , TO_CHAR(0.129, '0.00')    -- '0.13', 소수 이하 2자리 표기
  FROM
      DUAL;
...
SELECT SYSDATE              -- YY/MM/DD 형식으로 표시하지만 시간 데이터도 가지고 있음
...
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN TO_DATE(900101) AND TO_DATE(991231);

-- 2)
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
FROM EMPLOYEE
WHERE TO_DATE(HIRE_DATE, 'YY/MM/DD') BETWEEN TO_DATE('900101', 'YY/MM/DD') AND TO_DATE('991231' , 'YY/MM/DD');

-- 날짜는 성능때문에 VARCHAR2 (문자)로 처리하는 경우가 많다.
...
-- 날짜를 비교할 때는 TO_DATE를 사용하여 비교를 해야 한다.