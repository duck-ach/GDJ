-- 그룹(GROUP BY)
-- 1. GROUP BY절에서 지정한 칼럼의 데이터는 하나로 모아서 한 번만 조회가 된다.
-- 2. SELECT절에서 조회한 칼럼은 "반드시" GROUP BY절에 존재해야 함
...
-- 2. 그룹화 실패
-- EMPLOYEE_ID를 조회하려면 GROUP BY절에 EMPLOYEE_ID가 있어야 한다.
SELECT EMPLOYEE_ID
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;
...
-- 3. 그룹화 함수 사용
-- 집계함수(그룹함수)는 GROUP BY절에 해당 칼럼이 없어도 SELECT절에서 조회가능함
SELECT
    DEPARTMENT_ID
...
-- 4. 조건 지정
--      1) GROUP BY로 처리할 행(ROW)(모쑤)은 적을수록 성능에 유리함
...
-- 2) 소속된 사원수가 10명 이상의 부서의 연봉평균 조회하기(HAVING절만 가능)
...
/* 2. 동일한 부서번호(DEPARTMENT_ID로 그룹화하기
      동일한 부서번호를 가진 사원들을 직업아이디(JOB_ID)로 다시 그룹화하기
      즉, 부서별 직업아이디별로 그룹화하기
      부서별 직업아이디별로 그룹화하여 각 그룹의 사원 수 조회하기
      부서번호가 없는 사원은 제외하기
*/
SELECT
    DEPARTMENT_ID
...
-- 2. 동일한 지역(LOCATION_ID)으로 그룹화하여 존재하는 부서수 조회하기
--    부서수가 2 이상인 지역만 조회하기
SELECT
    LOCATION_ID
  , COUNT(*) AS 존재하는부서수
...
SELECT
    SUBSTR(DEPARTMENT_NAME, 1, 2) AS 부서명첫2글자
  , COUNT(*) AS 해당그룹의개수
  FROM DEPARTMENTS
...
SELECT
    SUBSTR(DEPARTMENT_NAME, 1, 2), COUNT(*) AS 해당그룹의개수
  FROM DEPARTMENTS
-- WHERE SUBSTR(DEPARTMENT_NAME, 1, 2) IN('IT', 'Co') AS 해당그룹의개수
 WHERE DEPARTMENT_NAME LIKE 'IT%' OR  DEPARTMENT_NAME LIKE'Co%'
 GROUP BY SUBSTR(DEPARTMENT_NAME, 1, 2);