...
-- 문제.
-- EMPLOYEE_ID 가 100인 회원의 FIRST_NAME, LAST_NAME, SALARY 정보를 읽어서 참조변수에 저장
DECLARE
...
BEGIN
    ...
    ELSE
        RESULT := 'E';
    END IF;
    DBMS_OUTPUT.PUT_LINE(SCORE || '점은 ' || RESULT || '등급 입니다.');
    
END;

-- 3) EMPLOYEE_ID가 150인 사원의 연봉을 참조해야
...
BEGIN
    ...
    ELSE 
        RES := '저연봉';
    END IF;
    DBMS_OUTPUT.PUT_LINE(SAL || '원은 ' || RES || '입니다.');
END;
...
-- FIRST_NAME과 LAST_NAME을 레코드 변수에 저장하고 해당값을 조회
...
BEGIN
    ...
    WHILE EMP_NO <= 206 LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(EMP_NO || ' ' || FULL_NAME.F_NAME || ' ' || FULL_NAME.L_NAME);
        EMP_NO := EMP_NO + 1;
    END LOOP; 
END;
...
BEGIN
    SELECT FIRST_NAME
     ...
     WHERE DEPARTMENT_ID = 50; -- 요구되는 FNAME은 1개인데 데이터는 6개여서 오류
EXCEPTION
