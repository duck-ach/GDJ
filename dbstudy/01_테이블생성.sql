...
/*
    ...
    1. 제약조건의 이름을 지정하지 않는 방법
        (SYS로 시작하는 임의의 제약조건이름이 지정) (SYS뒤에 숫자, 영문이 랜덤하게 붙음)
    2. 제약조건의 이름을 지정하는 방법
*/
...
CREATE TABLE USER_TBL(
    ...
    USER_ADDR VARCHAR2(30 BYTE) NULL,
    USER_TEL CHAR(13 BYTE) NULL, -- NULL은 허용하지만 중복은 허가하지 않는다.
    USER_GEN CHAR(1 BYTE) NULL,
    PRIMARY KEY(USER_ID),
...
CREATE TABLE USER_TBL(
    ...
    USER_ADDR VARCHAR2(30 BYTE) NULL,
    USER_TEL CHAR(13 BYTE) NULL UNIQUE, -- NULL은 허용하지만 중복은 허가하지 않는다.
    USER_GEN CHAR(1 BYTE) NULL
);
