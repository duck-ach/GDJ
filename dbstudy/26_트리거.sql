...
CREATE OR REPLACE TRIGGER TRIG1
    AFTER   -- AFTER, BEFORE
    UPDATE  -- UPDATE OR UPDATE OR DELETE
    ON DEPARTMENT -- DEPARTMENT 테이블 UPDATE할 때 동작함
    FOR EACH ROW
BEGIN
...
DELETE EMPLOYEE WHERE EMP_NO = 1001;
...
--1) 퇴사자 테이블만들기(EMPLOYEES 테이블과 동일한구조 데이터 없이 복사
DROP TABLE RETIRES;
...
ALTER TABLE RETIRES
    ADD RETIRE_ID VARCHAR2(30 BYTE) NOT NULL;
ALTER TABLE RETIRES
    ADD RETIRE_DATE DATE;

--3) RETIRE_ID 기본키 설정하기
ALTER TABLE RETIRES
    ADD CONSTRAINT PK_RETIRE_ID PRIMARY KEY(RETIRE_ID);
...
END RETIRE_TRIG;
--6) EMPLOYEES 테이블의 데이터 DELETE수행하기(RETIRE_TIRG 동작확인)
