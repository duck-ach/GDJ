...
--      3) INITCAP(칼럼) : INITIAL CAPITAL (첫글자만 대문자로 바꾼다.)
SELECT
    EMAIL
  , UPPER(EMAIL)
  , LOWER(EMAIL)
...
--      BEGIN은 INDEX가 아니다. (BEGIN은 1부터 시작함)
SELECT 
...
SELECT
    RPAD(SUBSTR(EMAIL, 1, 2), 5, '*') --오른쪽에 *를 채워서 5자리로 조회하겠다.
  FROM
    EMPLOYEES;
...
-- 데이터를 분석, 조작하는것은 JAVA가 아닌 ORACLE에서 함
