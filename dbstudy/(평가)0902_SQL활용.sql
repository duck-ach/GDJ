...
/*
WHERE TO_DATE(ORDER_DATE) = TO_DATE('20/12/31', ''YY/MM/DD'));
WHERE TO_DATE(ORDER_DATE) = TO_DATE('2020-12-31', 'YYYY-MM-DD'));
WHERE TO_DATE(ORDER_DATE) = TO_DATE('12/31-2020', 'MM/DD-YYYY'));
*/
DROP TABLE PRODUCT;
DROP VIEW V_PRODUCT;
/*
...
<< 데이터베이스 구조 >>
    1) 사원테이블
        EMPLOYEE (EMP_ID(PK), EMP_NAME, SALARY, DEPT_ID(FK))
    2) 부서테이블
...
SELECT EMP_ID, EMP_NAME, SALARY
  FROM EMPLOYEE
 WHERE DEPT_ID IN (SELECT DEPT_ID FROM DEPARTMENT WHERE DEPT_NAME = '총무부');
